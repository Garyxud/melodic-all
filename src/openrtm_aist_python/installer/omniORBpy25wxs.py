#!/usr/bin/env python
#
# @brief WiX wxs file generator for omniORBpy3.4-Python2.5
# @date $Date$
...
## Resource path
##
base_dir = os.getenv("OMNIORB_PY25")
if base_dir == None:
    base_dir = "C:\\distribution\\omniORBpy-3.4-Python2.5\\"
else:
...
for (path, files) in data:
    # wxs component name
    comp_name = path_to_comp_id(path, "py25")
    # wxs directory name
    dir_name = path_to_dir_id(path, "py25")
...
##
cmd = ["wxs",
       "-o", "omniORBpy25_inc.wxs",
       "-i", "omniORBpy25_inc.wxs.in"]
cmd += glob.glob("*.yaml")
makewxs.main(cmd)
